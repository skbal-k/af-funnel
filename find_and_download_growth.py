...
- Ac Sub Sub Region = LATAM-GROWTH
...
def main():
    with sync_playwright() as p:
        ...
        # ── Paso 3: Filtro Ac Sub Sub Region = LATAM-GROWTH ────────────────────────
        r = page.evaluate("""
            async () => {
                try {
                    const wb = tableau.VizManager.getVizs()[0].getWorkbook();
                    ...
                    for (const ws of wss) {
                        try {
                            await ws.applyFilterAsync('Ac Sub Sub Region', ['LATAM-GROWTH'], tableau.FilterUpdateType.REPLACE);
                            res.push(ws.getName() + ':ok');
        ...
        """)
        log.info("Ac Sub Sub Region=LATAM-GROWTH: %s", r)
        time.sleep(4)
        ...
        out_path = OUTPUT_DIR / "implementation_summary_growth.csv"
        ...
        growth_partners = ["COMPASS", "Accenture", "Deloitte", "Capgemini", "IBM",
                           "EVERYMIND", "C-Tech", "Wipro"]
        found = [p for p in growth_partners if p.upper() in content.upper()]
        if found:
            log.info("✓ Partners Growth encontrados: %s", found)
        else:
