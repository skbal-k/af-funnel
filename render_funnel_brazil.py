"""
Genera imagen del BRAZIL AF Implementation Funnel.
...
def get_scale_counts(partners):
    ...
    # Solo BRAZIL (columna Unnamed: 13 = sub-región)
    latam = df[df['Unnamed: 13'].str.upper().str.contains('BRAZIL', na=False)].copy()
...
csv_path = DOWNLOAD_DIR / "implementation_summary_brazil.csv"
...
ax.text(LEFT_PAD + (n_cols * COL_W) / 2, fig_h - 0.22,
        "BRAZIL AF Implementation Funnel",
...
# ── Guardar ───────────────────────────────────────────────────────────────────
out = DOWNLOAD_DIR / "brazil_funnel.png"
