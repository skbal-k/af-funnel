"""
Genera imagen del LATAM-EMERGING AF Implementation Funnel.
...
def get_scale_counts(partners):
    ...
    # Solo LATAM-EMERGING (columna Unnamed: 13 = sub-región)
    latam = df[df['Unnamed: 13'].str.upper().str.contains('LATAM-EMERGING', na=False)].copy()
...
csv_path = DOWNLOAD_DIR / "implementation_summary_emg.csv"
...
ax.text(LEFT_PAD + (n_cols * COL_W) / 2, fig_h - 0.22,
        "LATAM-EMERGING AF Implementation Funnel",
...
# ── Guardar ───────────────────────────────────────────────────────────────────
out = DOWNLOAD_DIR / "emg_funnel.png"
