"""
Genera imagen del LATAM-GROWTH AF Implementation Funnel.
...
def get_scale_counts(partners):
    ...
    # Solo LATAM-GROWTH (columna Unnamed: 13 = sub-región)
    latam = df[df['Unnamed: 13'].str.upper().str.contains('LATAM-GROWTH', na=False)].copy()
...
csv_path = DOWNLOAD_DIR / "implementation_summary_growth.csv"
...
ax.text(LEFT_PAD + (n_cols * COL_W) / 2, fig_h - 0.22,
        "LATAM-GROWTH AF Implementation Funnel",
...
# ── Guardar ───────────────────────────────────────────────────────────────────
out = DOWNLOAD_DIR / "growth_funnel.png"
